"""
Client Growth Report - Clean Streamlit Dashboard
Fixes:
1. Stable login using Streamlit secrets or hardcoded fallback
2. Removes temporary session password reset issue
3. Adds exchange rate input window
4. Uses the same exchange rate in report generation and email body
"""
...
h1, h2, h3 { color: #0099cc; }
</style>
""",
    unsafe_allow_html=True,
)

DEFAULT_USERNAME = st.secrets.get("DASHBOARD_USERNAME", "admin")
DEFAULT_PASSWORD = st.secrets.get("DASHBOARD_PASSWORD", "koenig1993")
...
def generate_report(file_24m_path, file_12m_path, exchange_rate):
    try:
        ...
        try:
            result = process_growth_report(df_24m, df_12m, str(output_file), exchange_rate=exchange_rate)
        except TypeError:
...
def run_report_and_email(file_24m_path, file_12m_path, key_prefix):
    ...
    with st.spinner("Generating report..."):
        success, report_file, result = generate_report(file_24m_path, file_12m_path, exchange_rate)
    if not success:
    ...
    if recipients:
        email_success, email_message = send_email_report(report_file, recipients, exchange_rate)
        if email_success:
...
if not st.session_state.authenticated:
    ...
    with col2:
        ...
        if submit:
            if username.strip() == DEFAULT_USERNAME and password.strip() == DEFAULT_PASSWORD:
                ...
                st.caption("Password is case-sensitive. Default fallback is admin / koenig1993 unless Streamlit Secrets override it.")
        st.markdown("---")
        st.caption("Password reset has been removed because session-based reset was temporary and caused login failures after logout.")
    st.stop()
...
with st.sidebar:
    ...
    st.caption(f"Current: INR amount ÷ {st.session_state.exchange_rate:g}")
    st.markdown("---")

    auto_files_exist = Path("data/RCB_24months.xlsx").exists() and Path("data/RCB_12months.xlsx").exists()
    ...
    if auto_files_exist:
        ...
        last_update = max(datetime.fromtimestamp(f24.stat().st_mtime), datetime.fromtimestamp(f12.stat().st_mtime))
...
""")
    ...
    if st.button("📊 Generate Report & Send Email", disabled=not (file_24m and file_12m), key="generate_manual"):
        data_dir = Path("data")
...
Client Growth Report Generator v2.1 | © 2026 Koenig Solutions
</div>
""", unsafe_allow_html=True)
